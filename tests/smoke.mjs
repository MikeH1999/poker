...
const bubblePromise = new Promise((resolve) => host.once("chat:bubble", resolve));
assert.equal((await emit(guest, "chat:send", { text: "准备好了吗？" })).ok, true);
const bubble = await bubblePromise;
assert.equal(bubble.playerId, guestId);
assert.equal(bubble.text, "准备好了吗？");
...
console.log(JSON.stringify({ room: created.roomId, hand: finished.hand.result.text, totalPoints: 3200, permissions: "ok", awayState: "ok", kick: "ok", runout: communityCounts, allInEquity: "ok", revealAfterHand: "ok", handName: "ok", autoStart: "ok", seatSelection: "ok", midHandJoin: "ok", reseatCount: "ok", chatBubble: "ok", actionTimeoutFold: "ok", actionTimeoutCheck: "ok" }));
host.disconnect();
guest.disconnect();
late.disconnect();
